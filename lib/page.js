...
module.exports = exports = function(options) {
  ...
  **/
  ChromePage.captureStep = async.queue(function(task, cb) {
    ...
    // check if not done
    if(VARS.running !== true && task.forced != true) return cb(null);
  ...
  **/
  ChromePage.bootstrap = function() {
    ...
    Network.requestIntercepted(({interceptionId, request}) => {
      ...
      var slug = uri.hostname + uri.pathname;
      if(VARS.hits[slug])
        VARS.hits[slug] = VARS.hits[slug] + 1;
      else
        VARS.hits[slug] = 1;

      if(VARS.hits[slug] > 20) {
    ...
    Network.requestWillBeSent((params) => {
      ...
      if(VARS.document) VARS.loadings.push(params.request.url);
    ...
    Network.dataReceived(({requestId, dataLength}) => {
      ...
      var index = VARS.loadings.indexOf(entry.request.url);
      if (index > -1) {
        VARS.loadings.splice(index, 1);
      }
    ...
    var queue = async.queue(function(task, callback) {
      ...
      getLoadStatus(function(err, result) {

        if(options.heartbeat) options.heartbeat();
        var loadStatus = (result || {}).value || 'loading';

        // debug
        // console.dir(VARS.loadings);
        
        // hide for now
        // console.log('status: ' + loadStatus + ' <' + VARS.responseCount + '/' + VARS.requestCount + '>')

        // the load flag
        var loadFlag = false;

        if(!VARS.lastHitTime) {
          VARS.lastHitTime = new Date();
          VARS.lastHit = VARS.loadings.join('');
          VARS.hits = 0;
        } else if(VARS.loadings.join('') != VARS.lastHit) {
          VARS.lastHit = VARS.loadings.join('');
          VARS.lastHitTime = new Date();
          VARS.hits = 0;
        }
        ...
        if((loadStatus == 'interactive' || loadStatus == 'loading') && VARS.lastHitTime &&  
            (new Date().getTime() - VARS.lastHitTime.getTime()) > 500) 
              VARS.hits++;

        // console.log(VARS.hits)
        // console.dir(VARS.loadings)
        ...
        if(task.forced === true) {
          ...
          loadFlag = true;
            ...
            VARS.document && 
              VARS.hits >= 8 && 
                (new Date().getTime() - VARS.lastResponseReceived.getTime()) > 500 && 
                  ((VARS.responseCount / VARS.requestCount)*100) > 90) {

          loadFlag = true;

        } else if(VARS.document && 
              VARS.loadings.length == 0 && 
        ...
        // stop capturing steps
        clearTimeout(VARS.loadTimer);
        clearInterval(VARS.timer);
        clearTimeout(VARS.timeoutTimer);
        clearTimeout(VARS.bigTimeoutTimer);
        clearInterval(VARS.recordEveryTimer);
        ...
        // set tracking urls
        VARS.trackingsurls = [].concat(VARS.loadings || []);
        ...
        if(options.recordEvery) {
          ...
          // finish up
          return ChromePage.captureStep.push({
    ...
    Network.loadingFailed(function(params){
      ...
      const entry = VARS.requests[params.requestId];
      if (entry) {
    ...
    Network.responseReceived(function(params) {
      ...
      if(params.type == 'Document' && 
        ...
        var index = VARS.loadings.indexOf(resource.request.url);
        if (index > -1) {
          VARS.loadings.splice(index, 1);
        }
    ...
    /**
    instance.on('Page.domContentEventFired', async function(params) {
      ...
      try {
        ...
        await Runtime.evaluate({ expression: `
          ...
          window.pmStatus = 'test';
          document.addEventListener('readystatechange', function(e) {
            window.pmStatus = e.target.readystate;
          });
    ...
    **/
    ...
    // promises.push(Network.setRequestInterceptionEnabled({enabled: true}))
    promises.push(Page.enable())
    /*promises.push(Page.addScriptToEvaluateOnNewDocument({ source: `

      window.pmStatus = 'loading';
      window.addEventListener( 'load', function( event ) {
        window.pmStatus = 'complete';
    });
    ...
    promises.push(Network.setCacheDisabled({
      ...
      cacheDisabled: options.disableCache === true
  ...
  **/
  ChromePage.end = _.once(function(params) {
